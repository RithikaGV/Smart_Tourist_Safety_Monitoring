...
async def ensure_indexes():
    ...
    # Admin panel lookups: the dashboard groups by category over a month window,
    # and the incident list filters by status + assigned officer.
    await db["incidents"].create_index([("category", 1), ("createdAt", -1)])
    await db["incidents"].create_index([("status", 1), ("createdAt", -1)])
    await db["incidents"].create_index("assignedOfficerId", sparse=True)

    await db["ai_alerts"].create_index("alertId", unique=True)
    await db["ai_alerts"].create_index([("status", 1), ("score", -1)])
    await db["ai_alerts"].create_index([("userId", 1), ("kind", 1), ("status", 1)])
