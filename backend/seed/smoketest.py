    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        r = await client.post(
            ...
            json={
                ...
                # Relative, not hardcoded - a fixed date silently becomes a "past"
                # trip once it passes, breaking this test months later.
                "travelDate": (date.today() + timedelta(days=14)).isoformat(),
                "travelTime": "12:30",
                "numberOfTravelers": 5,
                "travelType": "Family",
            },
        ...
        # mongomock doesn't implement $near. Depending on the transport it either
        # escapes as an exception or surfaces as a 500, so handle both shapes.
        near_unsupported = False
        try:
            r = await client.get(
            ...
            near_unsupported = r.status_code == 500 and "$near" in r.text
        except NotImplementedError:
            near_unsupported = True
